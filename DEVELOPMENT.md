...
**In English** — A Matrix-themed GitHub profile README. The green digital rain
banner is a hand-written animated SVG: 60 columns of falling katakana and digits
at independent speeds, a near-white leading glyph fading to dark green,
randomly flickering characters, a CRT scanline sweep, and a glowing wireframe
title. No canvas, no JavaScript — GitHub strips both from READMEs, so the whole
Matrix rain effect is pure SVG plus CSS keyframes, generated by a
standard-library-only Python script and committed by GitHub Actions.
Everything is self-hosted in this repository; the public README services this
kind of profile usually depends on were measured dead (see the notes below), so
there is nothing here that can break when someone else's free tier runs out.
...
README 里的图由 GitHub camo 代理并缓存，**缓存键是源 URL**。
文件内容变了但 URL 没变，camo 就会一直回旧图 —— 刷新浏览器、
清本地缓存都无效，因为缓存在 GitHub 那边。

所以 README 里三个自托管 SVG 的 URL 带着 `?v=N`。重新生成 SVG 后
主页不更新，把那个数字加一再 push，URL 变了 camo 就会重新抓。
